...
import { scrapeTakealot, getFeeCategories, createProduct, translateProductName } from '../api';
import type { ScrapeResult, FeeCategory } from '../types';
...
export default function ProductCreate() {
  ...
  const handleScrape = async () => {
    ...
    setError('');
    try {
      const result = await scrapeTakealot(url.trim());
      ...
      if (result.product_name && !chineseName) {
        try {
          const translated = await translateProductName(result.product_name);
  ...
  const handleSave = async () => {
    ...
    try {
      const product = await createProduct({
        ...
        purchase_cost_cny: purchaseCost ? parseFloat(purchaseCost) : null,
